...
async def vercel_update_db():
    ...
    await create_or_update_file(gh_access_token, gh_name, gh_email, repo_name,
                                "data.db", get_b64encoded_data(data), message)
...
def query_all(li, start: int = 0, end: int = -1, rule: str = "updated"):
    ...
    last_update_time = session.query(Post).limit(1000).with_entities(
        Post.createAt).all()
    last_update_time = max(x["createAt"].strftime("%Y-%m-%d %H:%M:%S")
...
def query_random_friend(num):
    ...
    if data:
        ...
        # data为空直接返回
        return {"message": "not found"}
    return friend_list_json[0] if len(
        friend_list_json) == 1 else friend_list_json
...
def query_post(
    ...
    session = db_interface.db_init()
    if link is None:
        user = session.query(Friend).filter_by(error=False).order_by(
            func.random()).first()
    ...
    posts = session.query(Post).filter(Post.link.like(
        "%{:s}%".format(domain))).order_by(
            desc(rule)).limit(num if num > 0 else None).all()
    session.close()
...
def query_friend_status(days):
    ...
    friend_status["total_lost_num"] = friend_status[
        "total_friend_num"] - friend_status["total_not_lost_num"]
...
def query_post_json(jsonlink, list, start, end, rule):
    ...
    headers = {
        "Cookie":
        "arccount62298=c; arccount62019=c",
        "User-Agent":
    ...
    last_update_time = max(
        x.createAt.strftime("%Y-%m-%d %H:%M:%S") for x in posts)
